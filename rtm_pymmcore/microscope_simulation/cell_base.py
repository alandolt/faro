...
def calculate_vertices(
    center: np.ndarray, angles: np.ndarray, r: np.ndarray
) -> np.ndarray:
...
def update_cell_physics(
    ...
    curvature_relax: float = 0.12,
    radial_relax: float = 0.08,
    ruffle_std: float = 0.01,
    seed: int = 0,
) -> tuple:
    """Update cell physics"""
    np.random.seed(seed)
...
class CellBase:
    ...
    def __init__(
        ...
        self.center = np.array(
            [rng.uniform(0, width), rng.uniform(0, height)], dtype=np.float64
        )
